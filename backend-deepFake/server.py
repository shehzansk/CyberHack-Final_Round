...
def detectFakeVideo(videoPath):
    ...
    # model = Model(2).cuda()
    model = Model(2)
    path_to_model = './model/model_final_data.pt'
    model.load_state_dict(torch.load(path_to_model, map_location=torch.device('cpu'), weights_only=False))
    model.eval()
...
def DetectPage():
    if request.method == 'GET':
        return jsonify({'message': 'Send a POST request with a video file'})
    if request.method == 'POST':
        try:
            ...
            video.save(os.path.join(os.getcwd(), 'Uploaded_Files', video_filename))
